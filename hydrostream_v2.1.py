...
* V2.1 adds a unit-aware, qualifier-aware ``PlausibilityFlag`` without
  deleting or altering flagged observations. It also writes the optional log
  continuously and retains a failed run workspace for diagnosis/recovery.
...
# Unit-aware plausibility limits. Boundaries are valid. A flag is raised only
# when the reported value/qualifier proves that the result lies outside the
# configured interval; rows are never removed by these rules.
PLAUSIBILITY_RULES = (
    {
...
def _plausibility_flag(frame: pd.DataFrame) -> pd.Series:
    """Return a strict boolean flag for definitively implausible results."""
    required = {"Test", "result", "ResultQualifier", "Unit"}
    missing = sorted(required - set(frame.columns))
    if missing:
        raise KeyError(
            "PlausibilityFlag requires columns: " + ", ".join(missing)
        )

    test = frame["Test"].astype("string").str.strip()
    unit = (
        frame["Unit"].astype("string").str.strip()
    ...
    qualifier = (
        frame["ResultQualifier"].astype("string").str.strip().fillna("")
    )
    exact = qualifier.isin(["", "="])
    flagged = pd.Series(False, index=frame.index, dtype=bool)

    for rule in PLAUSIBILITY_RULES:
        context = test.isin(rule["tests"]) & unit.eq(rule["unit"]) & result.notna()
        minimum = rule["minimum"]
        maximum = rule["maximum"]

        if minimum is not None:
            # <L proves a value below L; <=L can still equal the valid boundary.
            flagged |= context & (
                (exact & result.lt(minimum))
                | (qualifier.eq("<") & result.le(minimum))
                | (qualifier.eq("<=") & result.lt(minimum))
            )
        if maximum is not None:
            # >U proves a value above U; >=U can still equal the valid boundary.
            flagged |= context & (
                (exact & result.gt(maximum))
                | (qualifier.eq(">") & result.ge(maximum))
                | (qualifier.eq(">=") & result.gt(maximum))
            )
...
def _capacity_preflight(output_parent: Path, temp_parent: Path,
    ...
    if same_device:
        ...
        if available < required_output:
            raise RuntimeError(
                "Insufficient disk capacity for the conservative HydroStream "
                f"peak estimate: {required_output / 1024 ** 3:.1f} GiB required, "
                f"{available / 1024 ** 3:.1f} GiB free on {output_parent}. "
                "An archive-scale out-of-core run can temporarily hold the "
                "streaming staging CSV, final CSV/Parquet, and DuckDB spill "
                "files at the same time. chunksize controls RAM, not total "
                "disk use. Remove an obsolete .hydrostream-v2-run-* workspace "
                "only after confirming it is no longer needed, or set temp_dir "
                "to a larger separate volume."
            )
    else:
        ...
        if output_usage.free < required_output or temp_usage.free < required_temp:
            raise RuntimeError(
                "Insufficient output/temporary capacity for the conservative "
                f"peak estimate: output requires {required_output / 1024 ** 3:.1f} "
                f"GiB ({output_usage.free / 1024 ** 3:.1f} GiB free); DuckDB "
                f"temp requires {required_temp / 1024 ** 3:.1f} GiB "
                f"({temp_usage.free / 1024 ** 3:.1f} GiB free). chunksize "
                "controls RAM, not total disk use."
            )
    return {
        "raw_rows": raw_rows,
...
def _publish_artifacts_transactionally(
    ...
    The restored staging workspace is retained after failure so hours of
    completed work and the continuously written log are not silently lost.
    """
...
def hydrostream(
    ...
    *,
    ...
    previous_run_workspaces = sorted(
        path for path in out_dir.glob(".hydrostream-v2-run-*") if path.is_dir()
    )
    ...
    log_path: Optional[Path] = (
        run_dir / f"EA_processing_log_{mode}_v2.txt" if save_log else None
    )
    ...
    if capacity["same_filesystem"]:
        log(
            "  Required free disk: "
            f"{capacity['required_output_volume_bytes'] / 1024 ** 3:.2f} GiB "
            "(conservative peak; shared output/temp volume)"
        )
        log(
            "  Available disk   : "
            f"{capacity['output_free_bytes'] / 1024 ** 3:.2f} GiB"
        )
    else:
        log(
            "  Output required  : "
            f"{capacity['required_output_volume_bytes'] / 1024 ** 3:.2f} GiB "
            f"({capacity['output_free_bytes'] / 1024 ** 3:.2f} GiB available)"
        )
        log(
            "  Temp required    : "
            f"{capacity['required_temp_volume_bytes'] / 1024 ** 3:.2f} GiB "
            f"({capacity['temp_free_bytes'] / 1024 ** 3:.2f} GiB available)"
        )
    ...
    if previous_run_workspaces:
        log(
            "WARNING: found incomplete earlier run workspace(s). They are "
            "kept for diagnosis/recovery and may use substantial disk space:"
        )
        for previous in previous_run_workspaces:
            log(f"  {previous}")
        log()
    ...
    def _clean_chunk(df: pd.DataFrame,
        ...
        # Flag only results that are definitively outside the reviewed,
        # unit-specific limits. This never removes or alters an observation.
        df["PlausibilityFlag"] = _plausibility_flag(df)
        ...
        cols = [
            ...
            "UnitConversionCode", "PlausibilityFlag",
        ]
        if category_map:
    ...
    schema_crosswalk = pd.DataFrame([
        ...
        ("Plausibility flag", "derived", "derived", "PlausibilityFlag"),
    ...
    main_columns = [
        ...
        "Latitude", "Longitude", "PlausibilityFlag",
    ]
    if category_map:
    ...
    if using_duckdb:
        ...
        plausibility_count = int(con.execute(
            'SELECT count(*) FROM final_internal WHERE "PlausibilityFlag"'
        ).fetchone()[0])
        plausibility_by_test = con.execute("""
            SELECT "Test" AS Test, "Unit" AS Unit, count(*) AS flagged_rows
            FROM final_internal
            WHERE "PlausibilityFlag"
            GROUP BY "Test", "Unit"
            ORDER BY flagged_rows DESC, "Test", "Unit"
        """).fetchdf()
        ...
        plausibility_text = (
            df_all["PlausibilityFlag"].astype("string").str.strip().str.lower()
        )
        invalid_plausibility = ~plausibility_text.isin(["true", "false"])
        if invalid_plausibility.any():
            examples = sorted(
                plausibility_text.loc[invalid_plausibility].dropna().unique()
            )[:10]
            raise RuntimeError(
                "Invalid staged PlausibilityFlag values: " + ", ".join(examples)
            )
        df_all["PlausibilityFlag"] = plausibility_text.eq("true")
        if not source_identity_groups_df.empty:
            df_all = df_all.merge(
        ...
        plausibility_count = int(final_df["PlausibilityFlag"].sum())
        plausibility_by_test = (
            final_df.loc[final_df["PlausibilityFlag"]]
            .groupby(["Test", "Unit"], dropna=False)
            .size()
            .reset_index(name="flagged_rows")
            .sort_values(
                ["flagged_rows", "Test", "Unit"],
    ...
    output_schema_rows = [
        ...
        (
            "PlausibilityFlag",
            "boolean",
            "True only when the result is definitively outside a reviewed, unit-aware limit; rows are retained.",
        ),
    ]
    if category_map:
    ...
    if generate_stats:
        ...
        try:
            ...
            coverage = pd.DataFrame([
                ...
                ("Rows with PlausibilityFlag = True", plausibility_count),
            ...
            with pd.ExcelWriter(out_stats, engine="openpyxl") as writer:
                ...
                plausibility_by_test.to_excel(
                    writer, sheet_name="Plausibility_Flags", index=False
                )
    ...
    if generate_qa_report:
        ...
        try:
            ...
            plausibility_html = "\n".join(
                f"<tr><td>{html.escape(str(r['Test']))}</td>"
                f"<td>{html.escape(str(r['Unit']))}</td>"
                f"<td>{int(r['flagged_rows']):,}</td></tr>"
                for _, r in plausibility_by_test.iterrows()
            )
            if not plausibility_html:
                plausibility_html = (
                    "<tr><td colspan='3'>No definitively implausible values "
                    "were flagged.</td></tr>"
                )
...
<tr><td>Rows with PlausibilityFlag = True</td><td>{plausibility_count:,} ({plausibility_count/final_rows*100:.4f}%)</td></tr>
...
<h2>Plausibility flags</h2>
<div class="note">Flags are unit-aware and qualifier-aware. Boundaries are
valid, conductivity has no upper limit, and flagged rows remain in the dataset.</div>
<table><tr><th>Test</th><th>Unit</th><th>Flagged rows</th></tr>{plausibility_html}</table>
...
<h2>Top units</h2><table><tr><th>Unit</th><th>Rows</th><th>%</th></tr>{_html_rows(top_units,'Unit')}</table>
    ...
    completion_lines = [
        ...
        f"  Plausibility flags     : {plausibility_count:,}",
    ...
    if duck_tmp_dir != run_dir / "duckdb_tmp" and duck_tmp_dir.exists():
        try:
            ...
            print(
                "WARNING: outputs were published, but the external DuckDB "
                f"temporary workspace could not be removed: {duck_tmp_dir} ({exc})"
            )
    if run_dir.exists():
        try:
            ...
            print(
                "WARNING: outputs were published, but the intermediate "
                f"workspace could not be removed: {run_dir} ({exc})"
            )
    ...
    return {
        ...
        "data_quality": {
            ...
            "records_with_plausibility_flag": int(plausibility_count),
